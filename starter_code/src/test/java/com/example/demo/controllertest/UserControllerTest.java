...
class UserControllerTest {
	...
	userRequest.setUsername("Udacity2");
	...
	mockMvc.perform(MockMvcRequestBuilders.get("/api/user/{username}", "Udacity2")
		.accept(MediaType.APPLICATION_JSON).header("Authorization", request.getParameter("Authorization")))
		.andExpect(status().isOk());

	mockMvc.perform(
		MockMvcRequestBuilders.get("/api/user/{username}", "Udacity2").accept(MediaType.APPLICATION_JSON))
